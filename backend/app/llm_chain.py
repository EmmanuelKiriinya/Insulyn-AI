...
Your task:
Return ONLY a JSON object that follows this schema exactly:
...
⚠️ Rules:
- No extra commentary, markdown, or text outside the JSON.
- "feature_explanation" must always be a string, never an object.
- If you don’t know a value, still return a placeholder string (e.g. "Not available").
"""
    ...
    def prepare_prompt_data(patient_data: dict, ml_output: dict, bmi_category: str = None) -> dict:
        """Ensure all required prompt keys exist, with defaults if missing"""
        return {
    ...
    def generate_advice(self, patient_data: dict, ml_output: dict, bmi_category: str = None) -> str:
        """Generate structured diabetes advice using Groq LLM"""
        if not self.prediction_chain:
            return "LLM service temporarily unavailable. Please try again later."
